...
try{
  if(rawRender){
    if(rawMaterialProbe){
      ...
      renderer.clear(true,true,true);
      renderer.render(scene,camera);
    }else if(rawFixedProd){
      // BUILD 116: fixed camera + untouched production materials, direct renderer.
      // This isolates camera/control state from the material/shader pipeline.
      let marker=document.getElementById('hm-fixedprod-forensic-marker');
      if(!marker){
        marker=document.createElement('div');
        marker.id='hm-fixedprod-forensic-marker';
        marker.textContent='BUILD 116 • FIXED CAMERA / PRODUCTION MATERIALS';
        Object.assign(marker.style,{position:'fixed',left:'50%',top:'8px',transform:'translateX(-50%)',zIndex:'99999',padding:'8px 14px',borderRadius:'8px',background:'#0b6b5b',color:'#fff',font:'700 13px/1.2 monospace',letterSpacing:'.04em',pointerEvents:'none',boxShadow:'0 2px 10px rgba(0,0,0,.45)'});
        document.body.appendChild(marker);
      }
      controls.enabled=false;
      const fixedTarget=new THREE.Vector3(0,.72,30);
      ...
      renderer.setClearColor(0x101820,1);renderer.clear(true,true,true);renderer.render(scene,camera);
    }else if(rawFrameProd){
      // BUILD 122: use the exact known-positive auto-frame camera computation from
      // Build 114, but leave production materials completely untouched. This is the
      // first valid production-material test from a camera proven to see the world.
      let marker=document.getElementById('hm-frameprod-forensic-marker');
      if(!marker){
        marker=document.createElement('div');marker.id='hm-frameprod-forensic-marker';
        marker.textContent='BUILD 122 • AUTO-FRAME / PRODUCTION MATERIALS';
        Object.assign(marker.style,{position:'fixed',left:'50%',top:'8px',transform:'translateX(-50%)',zIndex:'99999',padding:'8px 14px',borderRadius:'8px',background:'#146c94',color:'#fff',font:'700 13px/1.2 monospace',letterSpacing:'.04em',pointerEvents:'none',boxShadow:'0 2px 10px rgba(0,0,0,.45)'});
        document.body.appendChild(marker);
      }
      controls.enabled=false;
      sky.visible=false;
      const bounds=new THREE.Box3(), box=new THREE.Box3();
      let meshCount=0;
      scene.updateMatrixWorld(true);
      scene.traverse(o=>{
        if(o===scene||o===sky)return;
        o.visible=true;
        if(o.isMesh&&o.geometry){
          meshCount++;o.frustumCulled=false;o.updateWorldMatrix(true,false);
          box.setFromObject(o);
          const finite=Number.isFinite(box.min.x)&&Number.isFinite(box.min.y)&&Number.isFinite(box.min.z)&&Number.isFinite(box.max.x)&&Number.isFinite(box.max.y)&&Number.isFinite(box.max.z)&&
            (box.max.x-box.min.x)<600&&(box.max.y-box.min.y)<600&&(box.max.z-box.min.z)<600;
          if(finite)bounds.union(box);
        }
      });
      scene.visible=true;
      const center=bounds.getCenter(new THREE.Vector3());
      ...
      renderer.setClearColor(0x101820,1);renderer.clear(true,true,true);renderer.render(scene,camera);
    }else if(rawArchitectureMapless){    }else if(rawArchitectureScalar){
      // BUILD 131: HERO ARCHITECTURE FRESH SCALAR MATERIAL ISOLATION.
      // This is a root-cause diagnostic, not a cosmetic pass. Builds 128-130 established:
      // geometry/camera/lights are valid; fresh StandardMaterials render correctly; restoring
      // authored material objects produces the dark-roof state; removing maps does not materially
      // change that state. Here we preserve ONLY authored scalar material properties while creating
      // entirely fresh StandardMaterials. No authored material object, map, shader hook, fog,
      // composer, camera, or global lighting state is reused by the diagnostic materials.
      let marker=document.getElementById('hm-archscalar-forensic-marker');
      if(!marker){
        marker=document.createElement('div');marker.id='hm-archscalar-forensic-marker';
        marker.textContent='BUILD 131 • HERO ARCHITECTURE FRESH SCALAR MATERIALS';
        Object.assign(marker.style,{position:'fixed',left:'50%',top:'8px',transform:'translateX(-50%)',zIndex:'99999',padding:'8px 14px',borderRadius:'8px',background:'#3f5368',color:'#fff',font:'700 13px/1.2 monospace',letterSpacing:'.04em',pointerEvents:'none',boxShadow:'0 2px 10px rgba(0,0,0,.45)'});
        document.body.appendChild(marker);
      }
      ...
      let meshCount=0,sourceMaterials=0,sourceMaps=0,sourceNormals=0;
      const scalarSummary=[];
      candidates.forEach(root=>{
        ...
        root.traverse(o=>{
          ...
          const copies=materials.map(m=>{
            ...
            sourceMaterials++;
            if(m.map)sourceMaps++;
            if(m.normalMap)sourceNormals++;
            const c=new THREE.MeshStandardMaterial({
              color:m.color?.clone?.()||new THREE.Color(0xffffff),
              roughness:Number.isFinite(m.roughness)?m.roughness:.5,
              metalness:Number.isFinite(m.metalness)?m.metalness:0,
              side:m.side??THREE.FrontSide,
              flatShading:!!m.flatShading,
              fog:false
            });
            // Preserve only emissive scalar state when the authored material actually uses it.
            if(m.emissive && m.emissiveIntensity>0){
              c.emissive.copy(m.emissive);
              c.emissiveIntensity=m.emissiveIntensity;
            }
            scalarSummary.push({
      ...
      window.__HEARTHMERE_FORENSIC_ARCH_SCALAR_STATS={
        rootCount:candidates.length,meshCount,sourceMaterials,sourceMaps,sourceNormals,
        ...
        hoverDisabled:true,freshStandardMaterials:true,
        copiedScalarProperties:['color','roughness','metalness','side','flatShading','emissive','emissiveIntensity'],
        scalarSummary
      };
      ...
      renderer.setClearColor(0x101820,1);renderer.clear(true,true,true);renderer.render(scene,camera);
    }else if(rawArchitectureDepth){
      // BUILD 125: architecture-only, shared Basic material, NORMAL DEPTH ENABLED.
      // Prior architecture tests disabled depth, so overlapping faces could collapse
      // into flat silhouettes. This test restores actual depth testing/writes.
      let marker=document.getElementById('hm-archdepth-forensic-marker');
      if(!marker){
        marker=document.createElement('div');marker.id='hm-archdepth-forensic-marker';
        marker.textContent='BUILD 125 • ARCHITECTURE DEPTH ENABLED';
        Object.assign(marker.style,{position:'fixed',left:'50%',top:'8px',transform:'translateX(-50%)',zIndex:'99999',padding:'8px 14px',borderRadius:'8px',background:'#0d6efd',color:'#fff',font:'700 13px/1.2 monospace',letterSpacing:'.04em',pointerEvents:'none'});
        document.body.appendChild(marker);
      }
      controls.enabled=false;sky.visible=false;scene.visible=true;
      const bounds=new THREE.Box3();let rootCount=0,meshCount=0;
      ...
      const mat=new THREE.MeshBasicMaterial({color:0x43ff88,side:THREE.DoubleSide,fog:false,transparent:false,depthTest:true,depthWrite:true});
      scene.traverse(o=>{if(o.isMesh){o.frustumCulled=false;if(o.visible){meshCount++;o.material=mat;}}});
      const center=bounds.getCenter(new THREE.Vector3()),size=bounds.getSize(new THREE.Vector3());
      const radius=Math.max(size.length()*.5,3),fov=THREE.MathUtils.degToRad(camera.fov),distance=Math.max(radius/Math.tan(fov/2)*1.25,8);
      camera.position.copy(center).addScaledVector(new THREE.Vector3(1,.52,1).normalize(),distance);
      camera.near=.05;camera.far=Math.max(500,distance+radius*4);camera.lookAt(center);camera.updateProjectionMatrix();
      scene.updateMatrixWorld(true);
      window.__HEARTHMERE_FORENSIC_ARCH_DEPTH_STATS={rootCount,meshCount,boundsEmpty:bounds.isEmpty(),center:center.toArray(),size:size.toArray(),camera:camera.position.toArray(),distance};
      renderer.setClearColor(0x101820,1);renderer.clear(true,true,true);renderer.render(scene,camera);
    }else if(rawArchitectureWire){
      // BUILD 126: architecture-only wireframe with normal depth. This removes
      // surface fill entirely and exposes the actual triangle topology/3D form.
      let marker=document.getElementById('hm-archwire-forensic-marker');
      if(!marker){
        marker=document.createElement('div');marker.id='hm-archwire-forensic-marker';
        marker.textContent='BUILD 126 • ARCHITECTURE WIREFRAME';
        Object.assign(marker.style,{position:'fixed',left:'50%',top:'8px',transform:'translateX(-50%)',zIndex:'99999',padding:'8px 14px',borderRadius:'8px',background:'#6f42c1',color:'#fff',font:'700 13px/1.2 monospace',letterSpacing:'.04em',pointerEvents:'none'});
        document.body.appendChild(marker);
      }
      controls.enabled=false;sky.visible=false;scene.visible=true;
      const bounds=new THREE.Box3();let rootCount=0,meshCount=0,vertexCount=0;
      scene.traverse(o=>{
        if(o===scene||o===sky)return;
        const isArch=o.userData?.architectureTier==='hero';
        if(isArch){o.visible=true;rootCount++;o.updateWorldMatrix(true,true);const b=new THREE.Box3().setFromObject(o);if(!b.isEmpty())bounds.union(b);}
      ...
      scene.traverse(o=>{
        ...
        if(o.visible){meshCount++;const pos=o.geometry?.attributes?.position;if(pos)vertexCount+=pos.count;o.material=new THREE.MeshBasicMaterial({color:0x43ff88,wireframe:true,fog:false,depthTest:true,depthWrite:true,transparent:false});}
      });
      const center=bounds.getCenter(new THREE.Vector3()),size=bounds.getSize(new THREE.Vector3());
      const radius=Math.max(size.length()*.5,3),fov=THREE.MathUtils.degToRad(camera.fov),distance=Math.max(radius/Math.tan(fov/2)*1.25,8);
      camera.position.copy(center).addScaledVector(new THREE.Vector3(1,.52,1).normalize(),distance);
      camera.near=.05;camera.far=Math.max(500,distance+radius*4);camera.lookAt(center);camera.updateProjectionMatrix();
      scene.updateMatrixWorld(true);
      window.__HEARTHMERE_FORENSIC_ARCH_WIRE_STATS={rootCount,meshCount,vertexCount,boundsEmpty:bounds.isEmpty(),center:center.toArray(),size:size.toArray(),camera:camera.position.toArray(),distance};
      renderer.setClearColor(0x101820,1);renderer.clear(true,true,true);renderer.render(scene,camera);
